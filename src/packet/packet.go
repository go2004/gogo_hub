...
//长度计算
func (p *Packet) Calculate() {
	packlen := p.Length()
	if packlen> PACKET_LIMIT {
		packlen= PACKET_LIMIT
	}
	p.data[0] = byte(packlen>>8 & 0xFF)
	p.data[1] = byte(packlen& 0xFF)
}
...
func Writer(cmd uint16) *Packet {
	pkt := &Packet{pos: 0}
	pkt.data = make([]byte,0, 128)
	pkt.WriteZeros(2)			//预留2个字节的长度
	pkt.WriteU16(cmd)		//写入命令号
	return pkt
}
